...
public class GraphAlgorithms {
    ...
    public static <T> List<Vertex<T>> bfs(Vertex<T> start, Graph<T> graph) {
        if (start == null || graph == null) {
            ...
            while (!queue.isEmpty()) {
                ...
                bfsList.add(v);
                for (VertexDistance<T> d : graph.getAdjList().get(v)) {
    ...
    public static <T> List<Vertex<T>> dfs(Vertex<T> start, Graph<T> graph) {
        if (start == null || graph == null) {
            ...
            dfsHelper(start, graph, visitedSet, dfsList);
            return dfsList;
        }
    }

    /**
     * Helper class for depth first search.
     *
     * @param v the current vertex
     * @param graph the graph passed in
     * @param vSet the visited set passed in
    ...
    private static <T> void dfsHelper(Vertex<T> v, Graph<T> graph,
        ...
        list.add(v);
        for (VertexDistance<T> d : graph.getAdjList().get(v)) {
            if (!vSet.contains(d.getVertex())) {
                dfsHelper(d.getVertex(), graph, vSet, list);
            }
        }
    }
            ...
            while (!queue.isEmpty() && visitedSet.size() < graph.getVertices().size()) {
                ...
                if (!visitedSet.contains(curr.getVertex())) {
                    ...
                    for (VertexDistance<T> d : graph.getAdjList().get(curr.getVertex())) {
                        if (!visitedSet.contains(d.getVertex())) {
                            queue.add(new VertexDistance<>(d.getVertex(),
                                    d.getDistance() + curr.getDistance()));
                        }
                    }
                }
            }
    ...
    public static <T> Set<Edge<T>> kruskals(Graph<T> graph) {
        if (graph == null) {
            ...
            while (!queue.isEmpty() && mstEdgeSet.size() < 2 * (graph.getVertices().size() - 1)) {
                Edge<T> edge = queue.remove();
                if (!disjointSet.find(edge.getU()).equals(disjointSet.find(edge.getV()))) {
                    mstEdgeSet.add(new Edge<>(edge.getU(), edge.getV(), edge.getWeight()));
